...
def process_packets(capture_file_path, website, id): 
...
def make_results_folders(dpyproxy, website, param_log_string = ""): 
    ...
    folder_name = os.path.join(os.getcwd(), "results", label, param_log_string, website.replace(".", "_"))
...
def capture_website_traffic(website, interface="en0", dpyproxy=False, result_folder="results"):
    ...
    try:
        ...
        if os.path.exists(capture_file):
            packet_metrics = process_packets(capture_file, website, id)
        ...
        metrics["param_string"] = param_log_string
        metrics["id"] = id
...
if __name__ == "__main__":
    ...
    for website in websites:
        print(f"\n{'='*50}\nProcessing {website}\n{'='*50}")
        result_folder = make_results_folders(dpyproxy, websites[0], param_log_string=param_log_string)
        print("Saving results to: ", result_folder)
        capture_website_traffic(website, interface, dpyproxy, result_folder)
